...
const WhyChoose = () => {
  ...
  const statsData = [
    { value: "98%", label: "Satisfaction Rate", icon: "😊", trend: "+12%" },
    { value: "15+", label: "Industry Partners", icon: "🤝", trend: "+5" },
    { value: "50+", label: "Expert Faculty", icon: "👩‍🏫", trend: "+8" },
    { value: "4.9★", label: "Google Rating", icon: "⭐", trend: "Excellent" },
  ];
            ...
            >
              <div className="relative bg-white rounded-2xl p-8 shadow-lg hover:shadow-2xl transition-all duration-500 hover:-translate-y-3 overflow-hidden">
                ...
                <div className="relative mb-6">
                  ...
                  ></div>
                  <div
                    className={`relative w-16 h-16 bg-gradient-to-br ${item.gradient} rounded-xl flex items-center justify-center text-3xl transition-all duration-500 group-hover:scale-110 group-hover:-rotate-6 shadow-md`}
                  >
                    {item.icon}
                  </div>
                </div>
                ...
                {/* Animated Bottom Bar */}
                <div
                  className={`absolute bottom-0 left-0 right-0 h-1 bg-gradient-to-r ${item.color} transform origin-left transition-transform duration-500 ${activeIndex === idx ? "scale-x-100" : "scale-x-0"}`}
                ></div>
              </div>
            </div>
          ))}
